...
function RewardCard({ reward, onRedeem }) {
  ...
  return <article className="reward-card"><div className="reward-card-image"><img src={image} alt=""/><strong>{pointsRequired.toLocaleString('en-IN')} PTS</strong></div><div className="reward-card-copy"><h3>{title}</h3><p>{description}</p><button onClick={() => onRedeem(reward)}>Redeem Now <span>→</span></button></div></article>;
}
