...
const envSchema = z.object({
  ...
  ADMIN_USERNAME: z.string().trim().min(3).max(100).default('Gac_Admin_Sejal'),
  ADMIN_PASSWORD_HASH: z.string().trim().min(20).default('$argon2id$v=19$m=65536,t=3,p=1$fjOxVKScRtZbaNHcDGRsZA==$oYj8UnRQURbiZ/Fs+1j+4pOqw6pFZmk70XBpb1BapSg='),
  SUPER_ADMIN_USERNAME: z.string().trim().min(3).max(100).default('GacAdmin'),
  SUPER_ADMIN_PASSWORD_HASH: z.string().trim().min(20).default('$argon2id$v=19$m=65536,t=3,p=1$aIx9+K/HxuLwW1E3yDUs0w==$IwPehHYgKxETeiyKM082h5JyIMA3TYa/XVSJ/tkHikc='),
  ...
  WHATSAPP_TEMPLATE_NAME_OTP: z.string().trim().default('otp'),
  WHATSAPP_TEMPLATE_NAME_REWARDS: z.string().trim().default('gac_booking_rewards'),
  WHATSAPP_TEMPLATE_NAME_TRIGGER: z.string().trim().default('trigger'),
  WHATSAPP_TRIGGER_BUTTON_URL: z.string().url().optional(),
